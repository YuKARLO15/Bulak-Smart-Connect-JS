    ...
    requirements: [
      {
        ...
        description:
          "Photocopy of any government-issued ID (e.g. Driver's License, Passport, UMID, PhilSys ID)",
        sampleImage: '/sample-images/valid-id-sample.jpg',
      },
      {
        title: 'Authorization Letter (if not the owner)',
        description:
          "If you're requesting for someone else, provide an authorization letter with valid IDs of both parties",
        sampleImage: '/sample-images/authorization-letter-sample.jpg',
      },
    ],
    fees: [{ item: 'Copy Issuance Fee', amount: '₱140.00 per copy' }],
  ...
  above18Marital: {
    ...
    requirements: [
      ...
      },
      {
        ...
        description:
          'Personal appearance or notarized affidavit explaining why personal appearance is not possible',
...
const RequirementBirthList = ({ applicationType }) => {
  ...
  return (
    <Box className="RequirementContainer">
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="md" className="SampleDialog">
        <DialogTitle className="SampleDialogTitle">{currentSample.title}</DialogTitle>
