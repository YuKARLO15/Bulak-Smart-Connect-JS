...
const CTCBirthCertificate = () => {
  ...
  const isFormComplete =
    uploadedFiles['Valid ID'] ||
    (formData?.purpose === 'Authorization Letter (if applicable)' &&
      uploadedFiles['Authorization Letter (if applicable)']);
      ...
      <Paper elevation={3} className="DocumentsPaperCTCBirth">
        ...
        <Box className="DocumentsListCTCBirth">
          ...
          <FileUpload
            ...
            onUpload={isUploaded =>
              handleFileUpload('Authorization Letter (if applicable)', isUploaded)
            }
            required={false}
          />
          <Typography variant="caption" className="DocumentDescriptionCTCBirth">
